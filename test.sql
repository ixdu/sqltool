-- Stored procedure definition script ServiceOrder_Post for MySQL
...
-- EXEC @ReturnStatus = ServiceOrder_RecalcCLR @CompanyID, @DivisionID, @DepartmentID, @OrderNumber
   SET @SWV_Error = 0;
   SET v_ReturnStatus = ServiceOrder_Recalc(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
...
-- Properly credit the customer account ->
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Order_CreditCustomerAccount2(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
...
-- Stored procedure definition script Invoice_AllServiceOrders for MySQL
-- Generated by (c) Ispirer SQLWays 4.0 Build 219 Licensed to STFB
-- Timestamp: Tue Jul 28 16:20:00 2015

DELIMITER //
DROP PROCEDURE IF EXISTS Invoice_AllServiceOrders;
//
CREATE       PROCEDURE Invoice_AllServiceOrders(v_CompanyID NATIONAL VARCHAR(36),
	v_DivisionID NATIONAL VARCHAR(36),
...
/*
Name of stored procedure: Invoice_AllServiceOrders
Method: 
	Creates invoices for all non-invoiced service orders
...
Called From:
	...
	Invoice_AllServiceOrders.vb

Calls:

	Invoice_CreateFromServiceOrder, Error_InsertError
...
*/
   ...
   TransactionTypeID = 'Service Order';
   ...
   WHILE NO_DATA = 0 DO
      ...
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
         ...
         ROLLBACK;
-- the error handler
         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Invoice_AllServiceOrders','',
         v_ErrorID);
         SET SWP_Ret_Value = -1;
      end if;
      SET NO_DATA = 0;
...
DELIMITER ;
select OrderNumber from orderheader;
